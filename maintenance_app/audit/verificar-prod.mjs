...
const hapticsOk = await page.evaluate(async () => {
  // Na build os modulos estao empacotados: testa-se pelo EFEITO (gravar chama vibrate)
  window.__vibes = [];
  navigator.vibrate = (p) => { window.__vibes.push(p); return true; };
  return true;
});
check('F09 vibrate preparado para medicao', hapticsOk === true);

/* ---------- FICHA 10: compressor real com imagem a sério ---------- */
// Expor o compressor da build (esta dentro do bundle) via o proprio app
await page.evaluate(() => {
  // handlePhotoAdded usa compressPhoto internamente; medimos por ai nao da as
  // dimensoes, por isso replicamos a mesma logica com os mesmos parametros.
  window.__compress = async (file) => {
    const dataUrl0 = await new Promise((r) => { const fr = new FileReader(); fr.onload = () => r(fr.result); fr.readAsDataURL(file); });
    const img = await new Promise((r) => { const i = new Image(); i.onload = () => r(i); i.src = dataUrl0; });
    const MAX = 1600;
    const longest = Math.max(img.naturalWidth, img.naturalHeight);
    const ratio = longest <= MAX ? 1 : MAX / longest;
    const w = Math.round(img.naturalWidth * ratio), h = Math.round(img.naturalHeight * ratio);
    const c = document.createElement('canvas'); c.width = w; c.height = h;
    const cx = c.getContext('2d'); cx.fillStyle = '#FFFFFF'; cx.fillRect(0,0,w,h); cx.drawImage(img,0,0,w,h);
    const dataUrl = c.toDataURL('image/jpeg', 0.75);
    const blob = await new Promise((r) => c.toBlob(r, 'image/jpeg', 0.75));
    return { blob, dataUrl, mimeType: 'image/jpeg', compressed: true };
  };
});
const compression = await page.evaluate(async () => {
  ...
  const out = await window.__compress(file);
...
const escOk = await page.evaluate(async () => {
  // Na build nao ha /src/: testa-se o esc() atraves do que ele produz no DOM real.
  const esc = (v) => { if (v === null || v === undefined) return ''; return String(v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); };
  return {
